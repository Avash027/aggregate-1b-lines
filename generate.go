...
func (ws WeatherStation) Measurement() float64 {
	return roundToNearestTenth(rand.NormFloat64()*10 + ws.meanTemperature)
}

func roundToNearestTenth(val float64) float64 {
	return float64(int(val*10+0.5)) / 10
}

func CreateMeasurements(numOfRecords int64) {
	start := time.Now()
	...
	// Define Weather Stations (similar to the Java version)
	...
	for i := int64(0); i < size; i++ {
		if i > 0 && i%50000000 == 0 {
			fmt.Printf("Wrote %,d measurements in %s ms\n", i, time.Since(start))
		}
	...
	fmt.Printf("Created file with %,d measurements in %s ms\n", size, time.Since(start))
}
